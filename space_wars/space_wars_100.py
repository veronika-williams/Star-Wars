...
def show_go_screen():
    ...
    draw_text(screen, "КОСМИЧЕСКИЕ", 56, WIDTH / 2, HEIGHT / 4)
    draw_text(screen, "ВОЙНЫ", 56, WIDTH / 2, HEIGHT / 3)
    draw_text(screen, "Стрелки - перемещаться, Пробел - стрелять", 22,
              WIDTH / 2, HEIGHT / 2)
    draw_text(screen, "Нажмите любую клавишу для продолжения", 24, WIDTH / 2, HEIGHT * 3 / 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
